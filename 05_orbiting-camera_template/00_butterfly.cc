...
class Camera
{
    ...
    public:
        ...
        {
            phi_loc=glGetUniformLocation(shaders.program, "phi_deg");
            theta_loc=glGetUniformLocation(shaders.program, "theta_deg");
            update_uniforms();
        }
        void drag(float dx, float dy)
        {
            ...
            if(theta_deg>90.0)theta_deg=90.0;
            if(theta_deg<-90.0)theta_deg=-90.0;
    ...
    private:
        ...
        {
            glUniform1f(theta_loc, theta_deg);
            glUniform1f(phi_loc, phi_deg);
        }
};
...
void handle(const sf::Event::MouseMoved& mouse, Scene& scene, Camera& camera)
{
    ...
    while (running)
    {
        while (const std::optional event = window.pollEvent())
        {
            ...
            /*else if (const auto* mouse = event->getIf<sf::Event::MouseButtonPressed>())
                handle (*mouse, scene, shaders);
            else if (const auto* mouse = event->getIf<sf::Event::MouseButtonReleased>())
                handle (*mouse, scene, shaders);*/
            else if (const auto* mouse = event->getIf<sf::Event::MouseMoved>())
                handle (*mouse, scene, camera);
        }
